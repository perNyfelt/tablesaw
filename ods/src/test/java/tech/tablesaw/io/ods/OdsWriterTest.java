...
public class OdsWriterTest {

  @Test
  public void testOdsWriter() throws IOException {
    ...
    table.write().usingOptions(OdsWriteOptions.builder(file).build());

    Table table2 = Table.read().usingOptions(OdsReadOptions.builder(file)
        .columnTypes(new ColumnType[]{ColumnType.INTEGER, ColumnType.LOCAL_DATE, ColumnType.DOUBLE}
        ).build());
    ...
    assertEquals(LocalDate.parse("2021-12-01"), table2.get(1, 1), "Second row, second column");
